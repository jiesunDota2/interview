...
5. Sorted Set（Zset)有序集合（跳表实现）：Sorted Set 多了一个权重参数 Score，集合中的元素能够按 Score 进行排列。可以做排行榜应用，取 TOP N 操作。
...
Redis中setnx不支持设置过期时间，做分布式锁时要想避免某一客户端中断导致死锁，需设置lock过期时间，在高并发时 setnx与 expire 不能实现原子操作(就是说setnx和expire是分开操作的，如果第一步执行完，redis重启，锁永远都在），SET lock_key client_id NX EX 10 这样就一步就是原子操作了
### Redis的LRU具体实现：
...
1. 在接口做校验（检查ID格式，限制访问频率）
2. 存null值（设置value是Null,较短的过期时间）
...
* B+树：在B树的基础上，将非叶节点改造为不存储数据纯索引节点，进一步降低了树的高度；此外将叶节点使用指针连接成双向链表，范围查询更加高效。
...
常见的限流算法有计数器、漏桶(leak)和令牌(bucket)桶算法。漏桶算法在分布式环境中消息中间件或者Redis都是可选的方案。发放令牌的频率增加(可以临时调节)可以提升整体数据处理的速度，而通过每次获取令牌的个数增加或者放慢令牌的发放速度和降低整体数据处理速度。而漏桶不行，因为它的流出速率是固定的，程序处理速度也是固定的。
